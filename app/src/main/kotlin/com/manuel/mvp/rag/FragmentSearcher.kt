 ...
 *
 * All query-building and relevance-ranking logic lives here (and in the shared [Fts5Query]), not
 * in [ContentDao] or [ContentDatabase] -- both of those are thin plumbing around a real
 * `androidx.sqlite` connection. This class only depends on the [FragmentRowSource] seam, which
 * is what lets `FragmentSearcherTest` (T005) exercise this exact production logic against a
 * plain JDBC SQLite connection instead of needing Android/Robolectric.
 *
...
class FragmentSearcher(private val source: FragmentRowSource) {
     ...
     * an empty list if nothing matches (including when [query] has no content words at all -- see
     * [Fts5Query.build]).
     */
    fun search(query: String, limit: Int = 5): List<ContentFragment> {
        val match = Fts5Query.build(query) ?: return emptyList()
        val rows = source.rawQuery(SEARCH_SQL, listOf(match, limit))
    ...
    private companion object {
        /**
         * `id` is UNINDEXED in the `fragments` FTS5 schema (see [ContentSchema]), so it is not
