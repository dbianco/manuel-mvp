...
- **The curated-answer set grew from 30 to 137 entries** (99 added from a 3rd/4th-grade guide, 8 more as natural follow-ups) with 2-3 extra phrasings added to every entry afterward for voice versatility — verified via `CannedAnswersTest` (every entry's own question resolves to itself; no new collisions from ~220 added phrasings) and live on this phone (`respuestas` table confirmed at `user_version` 5 with 137 rows after reinstall).
