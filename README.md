...
"Anita" spoken, or "Hablar ahora" tapped
                 ...
                 →  answer lookup (SQLite FTS5, OR-matched content words, best match wins):
                      1. curated Q&A  (content/preguntas_respuestas.json — 337 questions across
                         math, ciencias naturales, and geografía de Córdoba, each with several
                         phrasings, answered in short spoken-friendly Spanish)
...
| Content retrieval + answer | SQLite FTS5 (lexical search), via `androidx.sqlite:sqlite-bundled` | No vector database needed at this content scale; the bundled driver ships its own FTS5-capable SQLite, since stock Android's own SQLite doesn't have FTS5 compiled in (see [Verified on-device](#verified-on-device)). The spoken answer is a curated entry from `preguntas_respuestas.json` (337 questions × several phrasings each, including the digit forms and b/v confusions whisper actually produces, answered in voseo with operations spelled out in words so TTS reads them well) or, failing that, the best-matching lesson fragment's own text — no generation step. Three subjects, each grounded in its own markdown guide under `docs/content/`, one entry per guide question and citing it: 137 for math ([`preguntas-matematica-3-4-grado.md`](docs/content/preguntas-matematica-3-4-grado.md), 30 inicial/primario + 99 from a 110-question 3rd/4th-grade guide + 8 follow-ups), 100 for ciencias naturales ([`preguntas-ciencias-naturales-3-4-grado.md`](docs/content/preguntas-ciencias-naturales-3-4-grado.md): seres vivos, cuerpo humano, animales, plantas, materia, ciclo del agua, sistema solar, ecosistemas), 100 for geografía de la provincia de Córdoba ([`preguntas-geografia-cordoba.md`](docs/content/preguntas-geografia-cordoba.md): relieve, ríos y embalses, clima, ciudades, economía, historia). FTS5's bm25 shortlist is then re-ranked in Kotlin (`AnswerSearcher`): an entry whose own phrasing has exactly the question's content words wins (so "¿qué es sumar?" reaches the definition, not whichever entry repeats "sumar" most), and anything else needs at least two content words in common — one shared word ("hora", "sistema") is not the same question. `CannedAnswersTest` runs every one of the 337 entries' own question (the strongest collision check available without a phone, across three subjects' overlapping everyday vocabulary), the test protocol's questions, and paraphrases of every added set through the real FTS5 engine, and checks out-of-scope/ambiguous questions land on nothing |
...
- **Retrained the wake-word model for "Anita" and verified it live**: with a speaker playing the word through this phone's mic, "Anita" started a turn 4 out of 4 tries, while "Manuel" (the old wake word), a bare question with no wake word, an unrelated sentence containing "sumar" and "restar", and a sentence with similarly-sounding names ("Ana", "Anabela", "Marianita") triggered zero false positives — a real improvement over the original "manuel" model, which needed the **Hablar ahora** button often enough to exist at all (see the entry below).
- **The curated-answer set grew from 30 to 337 entries** in three passes: 99 added from a 3rd/4th-grade math guide, 8 more as natural follow-ups, then 2-3 extra phrasings added to every entry for voice versatility, then 100 ciencias naturales + 100 geografía de Córdoba entries (one per guide question each) — verified via `CannedAnswersTest` (every entry's own question resolves to itself across all 337, the strongest collision check available without a phone) and live on this phone (`respuestas` table confirmed at each step's `user_version`/row count after reinstall; screen showed the exact mistranscribed text for a question outside what the pipeline could answer, via the new `lastHeard` display below the buttons).
